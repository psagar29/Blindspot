...
import { Providers, ProviderError, FALLBACK_PROMPT, BULK_PROMPT } from './providers';
...
async function main() {
  ...
  if (command === 'seed-demo') { console.log(JSON.stringify(await seedDemo(config, process.argv.includes('--development-proxy')))); return; }
  ...
  if (command === 'cache-demo') {
    ...
    const checks = await providers.preflight() as Record<string, { creditsAvailable?: boolean }>;
    for (const provider of ['marble', 'tripo'] as const) {
      const prompt = provider === 'marble' ? FALLBACK_PROMPT : BULK_PROMPT;
      if (!checks[provider]?.creditsAvailable) { console.log(JSON.stringify({ provider, status: 'credits_unavailable' })); continue; }
      let job = await providers.start(provider, prompt);
      console.log(JSON.stringify({ provider, jobId: job.id, taskId: job.providerTaskId, status: job.status }));
      const deadline = Date.now() + 15 * 60 * 1000;
      while (job.status === 'running' && Date.now() < deadline) {
        await new Promise(resolve => setTimeout(resolve, 10000));
        try { job = await providers.poll(job.id); }
        catch (e) { console.log(JSON.stringify({ provider, status: 'poll_paused', code: e instanceof ProviderError ? e.code : 'NETWORK' })); continue; }
        console.log(JSON.stringify({ provider, status: job.status }));
      }
...
main().catch(e => {
  ...
  for (const secret of [config.worldKey, config.tripoKey, config.deployKey, config.modelKey]) if (secret) message = message.split(secret).join('[redacted]');
