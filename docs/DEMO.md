...
## Honest two-minute ground-first script

| Elapsed | Action and suggested line |
| --- | --- |
| 0:00–0:12 | “Before a robot enters a site, someone has to assess what it might miss. Blindspot exposes modeled blind spots before deployment.” No unsupported universal wire statistic. |
| 0:12–0:25 | Source photo → cached generated world. If it is the actual venue and permission/provenance exist, say so; otherwise name the prepared site. “We generated this world in [measured time]; this evaluation uses the cached result.” Show scale source/ruler. |
| 0:25–0:40 | Type supported sentence. Show three hazards and one quantitative justification. State “preset authoring” if no live model was used. Distinguish an authored stress-test cable from a cable confirmed in the source photo. |
| 0:40–1:02 | Split view. Cable in truth, modeled perception misses or detects too late. Show first-detection and required stopping distance. Let the actual outcome speak for three seconds. |
| 1:02–1:22 | Phone preset change. “Same route and geometry; a different sensor configuration.” Show queued → completed version, real coverage and false stops. Do not promise they necessarily move in opposite directions. |
...
C verifies laptop power/browser/GPU, operator lease, cached asset availability, Convex connection, phone QR link, permitted source image, published report on a fresh device and backup recording playback. Close debug env views and terminal output that might expose credentials. Preload splats before the timed demonstration. Disable system interruptions without disabling meaningful application error indicators.

Fallback order: real cached run → visibly labeled recorded playback → backup video + existing public report. Do not replay a canned result as a fresh simulation or fabricate online status.

## Prepared answers

- **Does this certify safety?** No. We found failures in a generated/approximated scene under declared sensor assumptions. A clean run is inconclusive.
- **Are the hazards really in the photo?** The report distinguishes photo-confirmed elements from authored stress hazards; the generated scene is not survey-grade evidence.
- **Can you trust splat depth?** Measurements use registered collider/proxy geometry and authored hazard dimensions; splats are appearance only.
